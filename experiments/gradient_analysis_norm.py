...
def compute_gradient_norms(model):
    ...
    for name, param in model.named_parameters():
        if param.grad is None:
            continue
        if 'W_r' in name or 'b_r' in name or 'gamma' in name or 'beta' in name:
...
def collect_gradients():
    ...
    logger.info("Collecting gradient norms with Norm...")
    ...
    np.save("logs/gradient_history_with_norm.npy", grad_history)
    ...
    plt.legend()
    plt.title('Gradient norms by component (with Norm)')
    ...
    plt.ylabel('||∇b|| / ||∇a||')
    plt.legend()
    plt.title('Gradient norm ratio (with Norm)')
    plt.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig('reports/figures/gradient_norms_with_norm.png', dpi=150)
    logger.info("Plot saved to reports/figures/gradient_norms_with_norm.png")
    plt.close()

    final_ratio = grad_history[-1, 1] / (grad_history[-1, 0] + 1e-8)
    mean_ratio = np.mean(grad_history[:, 1] / (grad_history[:, 0] + 1e-8))

    logger.info("GRADIENT ANALYSIS RESULTS (with Norm)")

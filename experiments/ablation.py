...
def main():
    ...
    logger.info("Training FULL model (with b-component)")
    full_model = DualClassifier(32, 16, EMBED_DIM, NUM_HEADS).to(device)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_acc > best_full_acc:
            ...
            torch.save(full_model.state_dict(), "logs/full_model.pt")
        if epoch % 10 == 0:
            logger.info(f"Epoch {epoch}: train_loss={train_loss:.4f}, val_acc={val_acc:.4f}")

    full_model.load_state_dict(torch.load("logs/full_model.pt"))
    _, full_acc = evaluate(full_model, test_loader, crit, device)
    logger.info(f"Full model test accuracy: {full_acc * 100:.2f}%")

    logger.info("Training ABLATION model (b-component frozen to 0)")
    ablation_model = DualClassifier(32, 16, EMBED_DIM, NUM_HEADS).to(device)
    ...
    logger.info(f"Trainable params in ablation model: {count_trainable_params(ablation_model)}")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_acc > best_ablation_acc:
            ...
            torch.save(ablation_model.state_dict(), "logs/ablation_model.pt")
        if epoch % 10 == 0:
            logger.info(f"Epoch {epoch}: train_loss={train_loss:.4f}, val_acc={val_acc:.4f}")

    ablation_model.load_state_dict(torch.load("logs/ablation_model.pt"))
